...
curl -X POST -i -H "X-Vault-Token:${VAULT_ROOT_TOKEN}" -d '{"db_name": "cassandra", "creation_statements":"CREATE USER '\''{{username}}'\'' WITH PASSWORD '\''{{password}}'\'' NOSUPERUSER; GRANT ALL PERMISSIONS ON ALL KEYSPACES TO {{username}}", "default_ttl": "2m", "max_ttl": "5m"}' ${VAULT_ADDR}/v1/database/roles/${DATABASE_ROLE}
