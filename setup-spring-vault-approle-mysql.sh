...
CUSTOM_ROLE_ID="movie-service-role-id"
DATABASE_USER="movie-user"

DATABASE_ROLE="movie-role"
DATABASE_ROLE_POLICY="movie-policy"
...
curl -X POST -i -H "X-Vault-Token: ${VAULT_ROOT_TOKEN}" -d '{"message": "Hello from movie-service"}' ${VAULT_ADDR}/v1/secret/movie-service
